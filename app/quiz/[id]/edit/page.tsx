...
export default function EditQuizPage() {
    ...
    api.get(`/quizzes/${id}`)
      .then((r) => {
        ...
        if (quiz.questions && quiz.questions.length > 0) {
          setQuestions(
            quiz.questions.map((q: { text: string; options: { text: string; is_correct: boolean }[] }) => ({
              text: q.text,
  ...
  const updateOption = (qi: number, oi: number, value: string) => {
    setQuestions((prev) =>
      prev.map((q, i) =>
        i === qi ? { ...q, options: q.options.map((o, j) => j === oi ? { ...o, text: value } : o) } : q
      )
    );
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(axiosErr.response?.data?.error || "Failed to update quiz");
    } finally {
      setLoading(false);
    }
  };
    ...
    <ProtectedRoute>
      <div className="max-w-3xl mx-auto px-4 py-10 space-y-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">Update your quiz details and questions</p>
        </div>
